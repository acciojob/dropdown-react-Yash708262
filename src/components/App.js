...
// Sample data for states, cities, and landmarks
const data = {
  states: [
    {
      name: "Madhya Pradesh",
      description: "A state in central India known for its rich cultural heritage.",
      cities: [
        {
          name: "Bhopal",
          description: "The capital city of Madhya Pradesh.",
          landmarks: [
            { name: "Bharat Bhavan", description: "A multi-arts complex." },
            { name: "Upper Lake", description: "A beautiful lake in Bhopal." }
          ]
        },
        {
          name: "Indore",
          description: "A city known for its food and commerce.",
          landmarks: [
            { name: "Rajwada", description: "A historical palace." },
            { name: "Indore Museum", description: "A museum showcasing local history." }
          ]
        }
      ]
    },
    {
      name: "Maharashtra",
      description: "A state in western India known for its financial capital, Mumbai.",
      cities: [
        {
          name: "Mumbai",
          description: "The financial capital of India.",
          landmarks: [
            { name: "Gateway of India", description: "A historic monument." },
            { name: "Marine Drive", description: "A famous boulevard." }
          ]
        },
        {
          name: "Pune",
          description: "A city known for its educational institutions.",
          landmarks: [
            { name: "Aga Khan Palace", description: "A historical landmark." },
...
const App = () => {
  ...
  const handleStateChange = (e) => {
    ...
    setSelectedCityIndex(0); // Reset city selection when state changes
    setSelectedLandmarkIndex(0); // Reset landmark selection when state changes
  };

  const handleCityChange = (e) => {
    setSelectedCityIndex(e.target.value);
    setSelectedLandmarkIndex(0); // Reset landmark selection when city changes
  };
  ...
  return (
    <div className="App">
      ...
      <div>
        ...
        <select id="landmark" value={selectedLandmarkIndex} onChange={(e) => setSelectedLandmarkIndex(e.target.value)}>
